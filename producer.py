...
def main():
    ...
    try:
        ...
        for obj in objects:
            if 'nyc_taxi_files' in obj.object_name and obj.object_name.endswith('.csv'):
                ...
                try:
                    ...
                    for index, row in data.iterrows():
                        try:
                            ...
                            # Convert row to JSON
                            record = row.to_json()
                            record_bytes = record.encode('utf-8')  # Corrected 'encodes' to 'encode'
                            record_stream = io.BytesIO(record_bytes)
                            record_stream_len = len(record_bytes)  # Corrected the length calculation
        ...
        if processed_objects == 0:
            logger.warning(f"No objects containing 'nyc_taxi_files' with '.csv' extension found in bucket '{bucket_name}'.")
